...
QRNN is available open source software quantnn \citep{} implemented with PyTorch backend \citep{paszke2017automatic}. The optimal configuration implemented for this study is described in Appendix~\ref{app:qrnn_conf}
...
Additionally, we also explored the effective range of $\rho$ for evans snow aggregate. These results are described in Appendix~\ref{}. 
...
For surfaces covered with snow, the PDs consist of a distinct arch type distribution with maximum PDs around 10\,K and a weak arm consisting of PDs as high as 50\,K. The arm type structure is present only in the observations. 
...
\subsection{Evans Snow aggregate}     %% Appendix A1, A2, etc.
(Figure~\ref{fig:divergence_esa}). The distribution of observed and simulated distributions are shown in Fig~\ref{}. In all experiments, as seen for large plate aggregate, the clear-sky TBs match quite well with the observations. On the other hand, while varying level of agreement is expected for cloudy cases, all experiments fall short of simulating the coldest TBs below 150\,K. The missing simulated Tbs lead to heavy penalties 
with respect to the observations, and is evident as a sharp spike in the divergences around 150\,K. When $\rho = 1.2$ and 1.3, the simulations can reproduce the PDs associated  TBs 


When $\rho$ = 1, the TBs between 150 and 200\,K cannot cover the arch region hence the divergence is higher.  the  $\rho$ from a truncated normal distribution or a uniform distribution gives comparable results. 

Further, with evans snow aggregate, the coldest TBs simulated are around 140\,\,K, while with large plate aggregate TBs as low as 100\,\,K are also simulated. 

he divergence estimates between the two distributions are quite similar to the ones obtained with large plate aggregate.
