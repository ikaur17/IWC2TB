...
aro = xarray.open_dataset("jan2020_IWP_lpa_v.nc")
tro = xarray.open_dataset("jan2020_IWP_lpa_vh.nc")

lsm0 = xarray.open_dataset("jan2020_lsm0.nc")
...
mask = (aro.lat.data > 40) & (aro.lat.data < 60)
mask1 = lsm0.lsm0.data == 8
...
ax.plot(bin_center, ahist,  label = "ARO VH")
ax.plot(bin_center, thist,   label = "TRO VH")
ax.set_yscale("log")
ax.set_xscale("log")
ax.legend()
...
ax[0].plot(bin_center, ahist,  label = "ARO VH")
ax[0].plot(bin_center, thist,   label = "ARO V")
ax[0].set_yscale("log")
ax[0].set_xscale("log")
ax[0].legend()
...
for itype, i in zip(["water", "land", "snow", "seaice"],[0, 1, 2, 3]):
    ...
    lsmmask = np.logical_and(lsmmask, mask)
...
for i in [0]:
    ...
    mask    = aro.lsm.data.ravel() >= i
    nanmask_a = np.logical_and(nanmask, mask)

    aiwp0, aiwp0c = zonal_mean(lat[nanmask_a], aiwp[nanmask_a], latbins)
    
    nanmask = ~np.isnan(tiwp)
    mask    = tro.lsm.data.ravel() >= i
    nanmask_t = np.logical_and(nanmask, mask)
    
    
    
    tiwp0, tiwp0c = zonal_mean(lat[nanmask_t], tiwp[nanmask_t], latbins)
    ...
    ax.set_title(i)
    #ax.set_xlim([0, 0.35])
    fig.savefig("Figures/zonal_mean_aro_tro_" + str(i) + "_all.png", bbox_inches = "tight")
...
ax = ax.ravel()
pd = (tb[:, :, 0] - tb[:, :, 1])
cs = ax[0].scatter(tb[:, :, 0][mask][::250], pd[mask][::250], 
...
ax[0].set_ylim([-10, 60])
ax[0].set_title("ARO")
cs = ax[1].scatter(tb[:, :, 0][mask][::250], pd[mask][::250], 
