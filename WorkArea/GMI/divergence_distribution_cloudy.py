...
th = three_sigma(tb_gmi1[:, :, 2])
mask1 = tb_gmi1[:, :,  2] < th

tb_gmi = tb_gmi1[mask1, :]
...
matfiles  = sorted(matfiles1 + matfiles2 + matfiles3)

gmi                     = GMI(matfiles[:])
ta_aro1                  = gmi.ta_noise

th = three_sigma(ta_aro1[:, 3])
mask2 = ta_aro1[:, 3] < th

ta_aro = ta_aro1[mask2, :]
...
ta_tro1                  = gmi_tro.ta_noise

th = three_sigma(ta_tro1[:, 3])
mask3 = ta_tro1[:, 3] < th

ta_tro = ta_tro1[mask3, :]
...
#%%
mask = (gmi.pratio[mask2] > 1.25) & (gmi.pratio[mask2] < 1.35) 
ta_tro_13 = ta_aro[mask]


mask = (gmi.pratio[mask2] > 1.15) & (gmi.pratio[mask2] < 1.25) 
ta_tro_12 = ta_aro[mask]

mask = (gmi.pratio[mask2] > 1.35)  
...
a11_14 = scipy.spatial.distance.jensenshannon( counts_tro, counts_gmi)
a12_14 = scipy.spatial.distance.jensenshannon( counts_tro.T, counts_gmi.T)

cc = cols[5]
a12_14 = gaussian_filter(a12_14, sigma)
a11_14 = gaussian_filter(a11_14, sigma)
ax[0].plot(xcenter, a12_14**2, label = r"$\rho$ = 1.4", c= cc, linewidth = 2.5)
ax[1].plot(ycenter, a11_14**2, label = r"$\rho$ = 1.4", c= cc, linewidth = 2.5)
