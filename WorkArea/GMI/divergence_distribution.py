...
im = (lsm_gmi == 1)
plot_contours(tb_gmi[im, :], ax[0] )
im = (lsm_gmi >= 3) & (lsm_gmi <= 7)
plot_contours(tb_gmi[im, :], ax[1] )

for i in range(2):
    ax[i].grid("on", alpha = 0.2)
    ax[i].set_xlim([60, 290])
    ax[i].set_ylim([-5, 35])

ax[1].legend(markerscale=3.5, fontsize = 19)
...
xbins = np.arange(50, 280, 4)
ybins = np.arange(-5, 30, 0.5)
...
ax[0].scatter(tb_gmi[im,  0].ravel()[::5],
           tb_gmi[im, 0].ravel()[::5] - tb_gmi[im,  1].ravel()[::5],
           label = 'Observation', c = c2, s = 2)




counts, xbins, ybins=np.histogram2d(tb_gmi[im,  0].ravel()[::5],
           tb_gmi[im, 0].ravel()[::5] - tb_gmi[im, 1].ravel()[::5],
                                        bins=(xbins, ybins), density = True)
sigma = 0.005
counts = gaussian_filter(counts, sigma)
...
contourlist[0] = np.array((240.,30.,30.))/255     # 5%     (red)
contourlist[1] = np.array((40.,40.,40.))/255       # 95%     (black)
cmap_contour = mpl.colors.ListedColormap(contourlist)
...
ax[0].contour(x[:-1,:-1], y[:-1,:-1], CT, cmap=cmap_contour, norm=norm_contour)
#ax[0].contour(x[:-1,:-1], y[:-1,:-1], CT, level = CT_perc[1], color = "k")
#ax[0].contour(x[:-1,:-1], y[:-1,:-1], CT, level = CT_perc[1], color = "k")

sigma = 0.08 # this depends on how noisy your data is, play with it!

ax[0].contour(counts.transpose(), [np.percentile(counts.T, 50),
                                   np.percentile(counts.T, 95)],
...
counts = gaussian_filter(counts, sigma)
ax[1].contour(counts.transpose(), level = [], extent=[xbins.min(),xbins.max(),
                ybins.min(),ybins.max()],linewidths=3,
...
cs_gmi = ax[2].contour(counts.transpose(),extent=[xbins.min(),xbins.max(),
                ybins.min(),ybins.max()],linewidths=3,
                linestyles='solid', colors = ['blue', 'red'], locator=ticker.LogLocator(),
